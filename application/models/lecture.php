...
//function getTeacherinfo($lr_seq) {
//        $member_col = 'taecher.mb_nick as teacher_name, taecher.mb_profile as teacher_profile,  member.mb_nick as member_name, member.mb_profile as member_profile, member.mb_skypeId as member_skypeId';
 ...
 //       $this->db->select('lr_lec_mb_no');

//       $this->db->join('g4_member taecher', 'taecher.mb_no = lr_lec_mb_no', 'LEFT');
//        $this->db->where(array('lr_seq' => $lr_seq));
 //       $ret = $this->db->get('g4_member')->row();
        // $sql = $this->db->last_query();
        // error_log('scheduel query === '.$sql);
 //     $sql = "select mb_comm_tool, mb_skypyId from g4_member where mb";
 //       $stmt = $this->db->query ( $sql );
//        return $stmt->result ();
//        return $ret;
 //   }
...
//   
        $member_col = 'teacher.mb_nick as teacher_name, teacher.mb_profile as teacher_profile, teacher.mb_comm_tool as teacher_tool, teacher.mb_skypeId as teacher_skypeId, member.mb_nick as member_name, member.mb_profile as member_profile,member.mb_skypeId as member_skypeId';
        $resevation_col = 'lr_seq, lr_lec_cd, lr_tsd_strtime, lr_package_koin, lr_group_yn, lr_tlp_id, lr_status, lr_comm_tool, lr_skypeId';
        $this->db->select($resevation_col.' , '.$member_col);
        $this->db->join('g4_member teacher', 'teacher.mb_no = lr_lec_mb_no', 'LEFT');
